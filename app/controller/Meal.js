  ...
  try {
    ...
    if (type === 'today') {
      ...
      return (
        ...
        }요일`.replace('수요일', '수요일 [잔반없는날]') +
        '\n\n' +
        meal[String(today.getDate())]
          .replace(/[,]/g, ', ')
          .replace(/[.]/g, '')
          .replace(/[0-9]/g, '')
          .replace('[석식]', '\n[석식]')
      ...
      return (
        `${tomorrow.getMonth() + 1}월 ${tomorrow.getDate()}일 ${
          this._week[tomorrow.getDay()]
        }요일`.replace('수요일', '수요일 [잔반없는날]') +
        '\n\n' +
        meal[String(tomorrow.getDate())]
          .replace(/[,]/g, ', ')
          .replace(/[.]/g, '')
          .replace(/[0-9]/g, '')
          .replace('[석식]', '\n[석식]')
...
Meal.getWeek = async function (date) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    week.push({
      date: `${month}월 ${stringDate}일 ${
        this._week[date.getDay()]
      }요일`.replace('수요일', '수요일 [잔반없는날]'),
      meal: meal[stringDate]
        .replace(/[,]/g, ', ')
        .replace(/[.]/g, '')
        .replace(/[0-9]/g, '')
        .replace('[석식]', '\n[석식]')
